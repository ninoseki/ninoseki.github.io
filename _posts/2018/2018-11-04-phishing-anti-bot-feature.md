...
これまで述べてきたようなアクセス制御をスクリプト(主に PHP)で行うパターンが存在します。
...
一部のフィッシングサイトでは、HTTP レスポンスの中身(= HTML)を暗号化することで、検索エンジンにクローリングされたとしてもその中身が容易に分析されないよう対策をしています。

具体的には、AES を使用して暗号化を行い、クライアントサイトの JS 側でそれを復号するという実装があります。
